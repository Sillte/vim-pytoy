...
from pytoy.shared.lib.events.infra.autocmd.vim_autocmd import EmitSpec, Group, PayloadMapper, VimAutocmd
...
class AutoCmdManager:
    ...
    def register(
        ...
        if group in self._autocmds:
            ...
            if autocmd.event_spec != emitter_spec or autocmd.payload_mapper != payload_mapper:
                raise ValueError(
                    f"Already `group` is registered, but `emitter_spec` or `argument_specs` are diffrent, "
                    f"{group=}, {emitter_spec=}, {autocmd.event_spec=}"
                )
            if owner != self._owners[group]:
                raise ValueError("The owner of group is different. ")
            return autocmd
        return self._create_autocmd(group, emitter_spec, payload_mapper, owner)
    ...
    def _create_autocmd(
        ...
        vim.command(cmd.make_command(self._dispatcher_vimfunc.impl_name))
    ...
    def dispose_owner(self, owner: object) -> None:
        groups = [k for k, value in self._owners.items() if value is owner]
        for group in groups:
            self.deregister(group)
