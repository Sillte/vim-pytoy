...
class CompletionContextMaker:
    ...
    def __call__(self, arg_lead: str, cmd_line: str, cursor_pos: int) -> CompletionContext:
        ...
        sub_command_token, current_token, prev_token = self._fetch_pivot_tokens(tokens, cursor_pos)
        ...
        if sub_command_token is None or current_token.index < sub_command_token.index:
            context = self._make_option_context(self.main_spec, current_token, prev_token, tokens, sub_command=None)
            if context:
                return context
        ...
        sub_tokens = [token for token in tokens if sub_command_token.index < token.index]
        if not sub_tokens:
            return CompletionContext(ContextType.NOCMP, tokens, current_token)

        context = self._make_option_context(self.main_spec, current_token, prev_token, tokens, sub_command=sub_command)
        if context:
            return context
    ...
    def _make_option_context(
        ...
        if opt_name is not None:
            return CompletionContext(ContextType.OPTION_KEY, tokens, current_token, sub_command=sub_command)
