...
class _OptionParser:
    def __init__(self, spec: MainCommandSpec | SubCommandSpec, arg_line: str):
        ...
        self.non_value_options = {option.name: option for option in spec.options if not option.expects_value}

        self.char_to_option: dict[str, str] = {
            option.short_option: option.name for option in spec.options if option.short_option
        }
...
def to_parsed_arguments(main_spec: MainCommandSpec, parsed_command: ParsedCommand) -> ParsedArguments:
    """[NOTE]:
    ...
    def _type_conversion(spec: MainCommandSpec | SubCommandSpec, options: dict[str, str]) -> dict[str, OptionType]:
        result = dict()
        ...
        for key, value in options.items():
            if key in name_to_spec:
                ...
                if opt_spec.type and value:
                    try:
                        value = opt_spec.type(value)  # type: ignore
                    except TypeError:
                        pass
            result[key] = value
        return result
