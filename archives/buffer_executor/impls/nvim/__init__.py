    ...
    def job_start(
        ...
        def wrapped_on_closed(*args):
            ...
            while time.time() - start < 0.5:
                if (not stdout_handler or stdout_handler.drained) and (not stderr_handler or stderr_handler.drained):
                    break
...
class NVimJobStartQueuePutter:
    ...
    def register(self) -> str:
        ...
        self._drained = True
        vimfuncname = PytoyVimFunctions.register(_on_stdout, name=f"{self.name}_{self.VIMFUNCTION_SUFFIX}")
...
class NvimBufferHandler:
    def __init__(self, queue_putter: NVimJobStartQueuePutter, queue_updater: QueueUpdater):
