...
class _OptionCandidatesProvider:
    ...
    def __call__(self, arg_lead: str, cmd_line: str, cursor_pos: int) -> list[str]:
        ...
        if self.context.completion_type == ContextType.OPTION_KEY:
            ...
            if option_name not in name_to_option:
                return [arg_lead]
            stem_candidates = self._from_completion(name_to_option[option_name].completion)
    ...
    def _get_options(self, sub_name: str | None) -> dict[str, OptionSpec]:
        ...
        name_to_subcommand = {sub_command.name: sub_command for sub_command in self.main_spec.commands}
    ...
    def _candidates_to_result(self, arg_lead: str, candidates: list[str] | None) -> list[str]:
        if not candidates:
            return [arg_lead]
...
class CompletionProvider:
    ...
    def __call__(self, arg_lead: str, cmd_line: str, cursor_pos: int) -> list[str]:
        ...
        if context.completion_type in {
            ...
            candidates = [
                sub_command.name for sub_command in self.main_spec.commands if sub_command.name.startswith(arg_lead)
            ]
