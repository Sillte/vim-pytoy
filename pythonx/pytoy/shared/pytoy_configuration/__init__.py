...
class PytoyConfiguration:
    ...
    def __init__(self, local_folder: str | Path | None = None, local_config_type: LocalConfigType | None = None):
    ...
    def _solve_local_config_mode(
        self, config_type: LocalConfigType | None, local_folder: str | Path | None = None
    ) -> LocalConfigType:
    ...
    def _solve_local_folder(
        self, local_folder: str | Path | None = None, local_config_type: LocalConfigType = "project"
    ) -> Path:
...
def _merge_copy(src_folder: Path, dst_folder: Path, remove_source: bool = False) -> None:
    if src_folder.resolve().is_relative_to(dst_folder.resolve()):
        raise ValueError(
            f"Destination folder {src_folder} is inside source {dst_folder}. Aborting for inappropriate usage."
        )
