...
from pytoy.ui.pytoy_buffer import BufferID, PytoyBuffer
...
class TTYExecutorProvider:
    @classmethod
    def make_executor(
        cls, app: TTYApplicationProtocol, spawn_option: SpawnOption | None = None
    ) -> TTYApplicationExecutorProtocol:
...
class TTYOrachestratorManager:
    ...
    def get_or_create(
        self, buffer: PytoyBuffer, app: TTYApplicationProtocol, spawn_option: SpawnOption | None = None
    ) -> TTYOrachestrator:
        ...
        if buffer_id in self._instances:
            ...
            if orchestrator.execeution_spec == query_execution_spec and orchestrator.alive:
                return orchestrator
            else:
