...
from pytoy.infra.events import Event, EventEmitter
from pytoy.infra.timertask import TimerTask
from pytoy.infra.vim_function import PytoyVimFunctions, VimFunctionName
from pytoy.tty_executor.impls.process_utils import find_children_pids, force_kill
        ...
        while self.alive:
            ...
            try:
                if isinstance(op, str):
                    ...
                    vim.session.threadsafe_call(vim.call, "chansend", self.job_id, payload)
...
class NeovimTTYExecutor(TTYApplicationExecutorProtocol):
    def __init__(
        ...
        self._events = ExecutorEvents(on_update=self._update_emitter.event, on_job_exit=self._jobexit_emitter.event)
    ...
    def snapshot(self) -> ConsoleSnapshot:
        ...
        return ConsoleSnapshot(timestamp=time.time(), content=content, config=self.console_configuration, cursor=cursor)
