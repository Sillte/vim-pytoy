...
def test_start_and_send_echo(terminal=None):
    ...
    joined = "\n".join(result_lines)
    assert "HelloTest" in joined, f"Expected 'HelloTest' in output, got:\n{joined} \n{terminal._line_buffer.chunk}"
    terminal.terminate()
...
def test_send_multiple_lines():
    ...
    output = "\n".join(result)
    assert "Line1" in output and "Line2" in output and "Line3" in output, "Expected all lines in output"
