...
def get_active_viewcolumn() -> int | None:
    api = Api()
    try:
        active_column = api.eval_with_return("vscode.window.activeTextEditor.viewColumn", with_await=False)
...
def get_active_uri() -> Uri | None:
    api = Api()
    try:
        uri = api.eval_with_return("vscode.window.activeTextEditor.document.uri", with_await=False)
    except Exception:
        return None
    return Uri(**uri)
